...
namespace LazyEntityFrameworkCore.Lazy.Proxy
{
    public class ProxyBuilder : IProxyBuilder
    {
        ...
        private ConcurrentDictionary<IEntityType, Func< DbContext, object>> _EmptyBuilders = new ConcurrentDictionary<IEntityType, Func<DbContext, object>>();
        ...
        public object ConstructProxy(IEntityType entityType, DbContext context)
        {
            Func<DbContext, object> builder = _EmptyBuilders.GetOrAdd(entityType, CreateEmptyBuilder);
            return builder(context);
        }

        private Func<DbContext, object> CreateEmptyBuilder(IEntityType entityType)
        {
            //IMemberMapper memberMapper = context.GetInfrastructure().GetService<IMemberMapper>();
            ...
            var newExpression =Expression.New(GetDeclaredConstructor(proxyType, new [] {typeof(DbContext)}), contextExpession);

            return Expression.Lambda<Func<DbContext, object>>(newExpression, contextExpession).Compile();
        }
